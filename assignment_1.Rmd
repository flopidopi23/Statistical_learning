...
estimate_threshold_via_percentile <- function(data, percentile = 0.90, distance_metric = "euclidean") {
  ...
  # Extract upper-triangular distance values
  #dist_values <- dist_1[upper.tri(dist_1)]
  
  # Fit normal distribution (estimate parameters)
  #mu <- mean(dist_values)       # Mean
  #sigma <- sd(dist_values)      # Standard deviation
  
  # Compute 95% confidence interval
  #threshold = qnorm(0.975, mean = mu, sd = sigma)  # 97.5% quantile
  #print(dist_values)
  #print(threshold)
...
knn_classifier = function(test_data, train_data, thresholds, dist_metric = "euclidean", k, knn_bool = T){
  ...
  for (i in 1:nrow(test_data)) {
    ...
    for (train_ids in names(thresholds)) { 
      print(names(thresholds))
      print(distances[names(distances) == train_ids])
      print(thresholds[train_ids])
...
classification_pipeline <- function(train_data,
  ...
  print(df_classification_binary)
...
knn_k_fold_cv <- function(data,
  ...
  all_results <- list()
  # Perform K-fold cross-validation the normal way
  ...
  unique_ids <- unique(data[, ncol(data)])
  all_results_out_of_sample = c()
  
  for (IDiter in unique_ids) {
    train_out_of_sample <- data[data[, ncol(data)] != IDiter, , drop = FALSE]  # Train set: all except current ID
    test_out_of_sample  <- data[data[, ncol(data)] == IDiter, , drop = FALSE]  # Test set: only current ID
  
    # Run the classification pipeline for out of sample data
    results_out_of_sample <- classification_pipeline(train_out_of_sample, 
                                       test_out_of_sample,
                                       estimate_func = estimate_func,
                                       percentile = percentile,
                                       knn_k = knn_k ,
    ...
    # Store results for this fold
    all_results_out_of_sample[[IDiter]] <- list(
      binary_false_positive_rate = results_out_of_sample$binary_false_positive_rate
    )
  }
  
  # Store means
  binary_false_positive_rate_mean <- mean(sapply(all_results_out_of_sample, function(x) x$binary_false_positive_rate))
...
```{r}
### K fold Cross Validation
knn_out_off_sample_cv <- function(data,
                          estimate_func, 
                          percentile, 
                          knn_k, 
                          dist_metric,
...
data_PC = data_asmatrix[,-ncol(data_asmatrix)] %*% PCA_result$"Eigen Vector"[,1:40]
