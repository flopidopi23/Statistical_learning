...
data_matriz <- apply(data_matriz, 2, as.numeric)
```
...
avg_scores <- knn_k_fold_cv(data_matriz,
                            folds,
                            k_CV = k_CV,
                            estimate_func = estimate_threshold_via_percentile,
                            percentile = 0.9,
...
```{r}
### Run PCA & prep data
pca_result <- PCA.fun(data_matriz, matrix_bool=T)
...
### Run KNN on FDA + PCA Data

```{r}
### Run FDA
result_FDA = FDA.fun(data_reduced,labels)
...
data_reduced_fda = cbind(data_reduced_fda,labels)
```


```{r}
### Run grid Search to optimize Hyperparameters
CV_grid_search_result <- CV_grid_search(data_reduced_fda,
                                        k_CV = 5,
                                        knn_k_values = c(3, 4, 5), 
                                        percentile_values = c(0.9, 0.95), 
                                        dist_metrics = c("euclidean", "manhattan"))
