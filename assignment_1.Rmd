...
create_knn_results <- function(train_data, 
                               test_data,                               
                               image_names,
                               true_ID,
                               dist_metric,
  ...
  # Estimate the thresholds using the training data
  thresholds <- estimate_thresholds(train_data, 
                                    estimate_func = estimate_func, 
                                    percentile = percentile)
  
  # Apply the KNN classifier to the test data
  results <- knn_classifier(test_data = test_data[, -ncol(test_data)], 
  ...
  # Score
  match_rate <- mean(final_dataframe$true == final_dataframe$predicted)
  fail_rate <- 1 - match_rate
  
  print(paste0("Match Rate:",match_rate))
  print(paste0("Mismatch Rate:", fail_rate))
...
image_names_test = image_names[-train_idx]
labels_test = labels[-train_idx]


results = create_knn_results(train_data = train_data_pca, 
                               test_data = test_data_pca,
                              image_names = image_names_test,
                              true_ID = labels_test,
...
#data_train = read_all_images("Training_alpha")
#data_test = read_all_images("Test_alpha")
...
labels_test_init = data_test_asmatrix[,ncol(data_test_asmatrix)]
labels_train = data_train_asmatrix[,ncol(data_train_asmatrix)]
...
results = create_knn_results(train_data = data_train_asmatrix, 
